...
const Navbar = () => {
  ...
  const handleCardNavToggle = () => setCardNavOpen(!cardNavOpen)
  const handleCardNavClose = () => setCardNavOpen(false)
  const handleAuthOpen = (mode) => { setAuthMode(mode); setAuthModalOpen(true) }
  ...
  return (
    <>
        ...
        <motion.nav
        className="fixed top-0 left-0 right-0 z-[1000] w-full px-sm py-sm md:px-md lg:px-lg bg-background-primary/75 backdrop-blur-[24px] border-b border-glass-border shadow-glass"
        initial={{ y: -100, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ duration: 0.6, ease: 'easeOut' }}
      >
        <div className="max-w-[1400px] mx-auto flex items-center justify-between h-[60px] gap-sm md:gap-md lg:gap-lg">
          <Link to="/" className="font-serif text-[1.1rem] md:text-[1.35rem] font-bold text-primary whitespace-nowrap transition-all duration-300">
            <motion.span
          ...
          <div className="flex items-center gap-xs md:gap-md">
            ...
            {isLoggedIn ? (
              ...
              <div className="flex items-center gap-xs">
                ...
                <motion.button
                  className="hidden sm:flex items-center justify-center p-sm px-md text-[0.85rem] font-sans font-medium bg-glass-surface border border-accent-purple/60 text-accent-purple rounded-full cursor-pointer transition-all duration-300 whitespace-nowrap hover:bg-accent-purple/10 hover:border-accent-purple hover:shadow-glow"
